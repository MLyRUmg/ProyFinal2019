...
<footer id="footer" itemscope="itemscope" itemtype="http://schema.org/WPFooter">
	...
	<div class="container">
			...
			if ( ! empty( $zerif_address_icon ) || ! empty( $zerif_address ) ) {
				...
				if ( ! empty( $zerif_address_icon ) ) {
					...
					if ( ! empty( $zerif_address ) ) {
						echo '<a target="_blank" href="https://www.google.com.gt/maps/place/' . wp_strip_all_tags( $zerif_address ) . '">';
					}
		...
		if ( ! empty( $zerif_socials_facebook ) || ! empty( $zerif_socials_twitter ) || ! empty( $zerif_socials_linkedin ) || ! empty( $zerif_socials_behance ) || ! empty( $zerif_socials_dribbble ) ||
			...
			echo '<div class="zerif-copyright-box"><a class="zerif-copyright" rel="nofollow">UMG </a>' . sprintf( __( 'Powered by Grupo 2 %s', '' ), '<a class="zerif-copyright" href="' . esc_url( '' ) . '" ' . $attribut_new_tab . ' rel="nofollow">' . __( '', 'zerif-lite' ) . '</a>' ) . '</div>';
